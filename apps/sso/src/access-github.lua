...
local function handle_token_uris(email, token, expires)
    ngx.log(ngx.ERR, inspect {uri, sso_uri, email, token, expires})
    if sso_uri == "/_token.json" then
    ...
    if sso_uri == "/_token.txt" then
        ngx.header["Content-type"] = "text/plain"
        ngx.say("email: " .. email .. "\n" .. "token: " .. token .. "\n" .. "expires: " .. expires .. "\n")
        ngx.exit(ngx.OK)
    end

    if sso_uri == "/_token.curl" then
        ngx.header["Content-type"] = "text/plain"
        ngx.say(
            '-H "OauthEmail: ' ..
                email .. '" -H "OauthAccessToken: ' .. token .. '" -H "OauthExpires: ' .. expires .. '"\n'
        )
        ngx.exit(ngx.OK)
    end
end
...
local function request_profile(token)
    ...
    ngx.log(ngx.ERR, inspect {res, err})
    if not res then
...
local function redirect_to_auth()
    ...
    local redirect_url = cb_url
    if uri_args["redirect_uri"] then
        redirect_url = cb_scheme .. "://" .. cb_server_name .. uri_args["redirect_uri"]
    end

    ngx.log(ngx.ERR, inspect {uri_args, redirect_url})
    return ngx.redirect(
...
local function authorize()
    ...
    ngx.log(ngx.ERR, "profile:" .. inspect(profile))
    if not profile then
        ngx.log(ngx.ERR, "got error during profile request: " .. profile_err)
        return ngx.exit(ngx.HTTP_FORBIDDEN)
    end

    if not token["expires_in"] then
    ...
    ngx.log(ngx.ERR, "email:" .. inspect(email))
    ...
    ngx.log(ngx.ERR, "cookie:" .. inspect(ngx.header["Set-Cookie"]))

    return ngx.redirect(uri_args["state"])
end

local function handle_signout()
    ngx.log(ngx.ERR, "handle_signout")
    if sso_uri == signout_uri then
...
if not is_authorized() then
    ngx.log(ngx.ERR, "not authorized")
    authorize()
end
ngx.log(ngx.ERR, "success authorized")

-- if already authenticated, but still receives a /_oauth request, redirect to the correct destination

if uri_args["redirect_uri"] then
    return ngx.redirect(uri_args["redirect_uri"])
end
if sso_uri == "/_oauth" then
    return ngx.redirect(uri_args["state"])
end
