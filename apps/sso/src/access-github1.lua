...
local function redirect_to_auth()
    ...
    if ngx.var.arg_redirect_uri then
        redirect_url = cb_scheme .. "://" .. cb_server_name .. ngx.var.arg_redirect_uri
    end
...
local function authorize()
    ...
    if uri_args["error"] then
        ngx.log(ngx.ERR, "received " .. uri_args["error"] .. " from https://github.com/login/oauth/authorize")
        return ngx.exit(ngx.HTTP_FORBIDDEN)
    end
    ...
    ngx.log(ngx.ERR, "cookie:" ..     ngx.header["Set-Cookie"] )
